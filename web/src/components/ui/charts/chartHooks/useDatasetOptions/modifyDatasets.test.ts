...
describe('modifyDatasets - percentage stack tests', () => {
  ...
  test('should handle percentage-stack with negative values', () => {
    const datasetsWithNegativeValues: DatasetOption[] = [
      {
        id: 'dataset1',
        label: [{ key: 'category', value: 'Series 1' }],
    ...
    // First point: 100 + 50 = 150 total (all positive)
    // 100/150 = 66.67%, 50/150 = 33.33%
    expect(result[0].data[0]).toBeCloseTo(66.67, 1);
    expect(result[1].data[0]).toBeCloseTo(33.33, 1);

    // Second point: -50 + 150 = 100 total (mixed)
    // -50/100 = -50%, 150/100 = 150%
    expect(result[0].data[1]).toBeCloseTo(-50, 1);
    expect(result[1].data[1]).toBeCloseTo(150, 1);

    // Third point: 200 + (-100) = 100 total (mixed)
    // 200/100 = 200%, -100/100 = -100%
    expect(result[0].data[2]).toBeCloseTo(200, 1);
    expect(result[1].data[2]).toBeCloseTo(-100, 1);

    // Check that tooltipData still reflects the original values
    expect(result[0].tooltipData?.[0]?.[0]?.value).toBe(100);
    expect(result[0].tooltipData?.[1]?.[0]?.value).toBe(-50);
    expect(result[0].tooltipData?.[2]?.[0]?.value).toBe(200);

    expect(result[1].tooltipData?.[0]?.[0]?.value).toBe(50);
    expect(result[1].tooltipData?.[1]?.[0]?.value).toBe(150);
    expect(result[1].tooltipData?.[2]?.[0]?.value).toBe(-100);
  });
});
