  ...
  const onSetFileView = useMemoizedFn(
    async ({
      ...
      if (!chatId) {
        ...
        const waitToOpen = !appSplitterRef.current?.isSideClosed('left') && chatId ? 250 : 0; //I used to wait 250ms, but I changed the app splitter and I think it fixed the bug
