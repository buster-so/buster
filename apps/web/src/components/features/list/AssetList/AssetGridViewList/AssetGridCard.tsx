...
export const AssetGridItem = React.memo((props: AssetGridItemProps) => {
  ...
  const cardContent = (
    <Link {...link} preload={false} className="h-full">
      <div className="group border rounded cursor-pointer hover:border-gray-dark! overflow-hidden h-full flex flex-col group-data-[state=open]:border-gray-dark">
        <div
          className={cn(
            'px-2.5 flex-1 pt-2.5 bg-item-select overflow-hidden',
        ...
        <div className="h-[60px] px-3 flex flex-col space-y-0.5 border-t group-hover:bg-item-hover/10 group-data-[state=open]:bg-item-hover/10 flex-shrink-0 justify-center bg-background">
