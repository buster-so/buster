...
interface MenuHeaderSearchProps<T> {
  text: string;
...
export const MenuHeaderSearch = <T,>({
  text,
  onChange,
  onSelectItem,
  onPressEnter,
  ...
  placeholder,
  className = '',
}: MenuHeaderSearchProps<T>) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const { onChange: onChangePreflight, onKeyDown: onKeyDownPreflight } = useMenuSearch({
    showIndex,
    onSelectItem,
    onChange,
  });
...
MenuHeaderSearch.displayName = 'MenuHeaderSearch';
