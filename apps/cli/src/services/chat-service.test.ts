...
describe('runChatAgent - message updates', () => {
  ...
  describe('SDK initialization', () => {
    it('should initialize SDK when messageId is provided and credentials exist', async () => {
      ...
      expect(credentials.getCredentials).toHaveBeenCalled();
    ...
    it('should auto-generate messageId when not provided and initialize SDK', async () => {
      const { createBusterSDK } = await import('@buster/sdk');
      const mockCreate = vi.fn().mockResolvedValue({ success: true });
      const mockUpdate = vi.fn().mockResolvedValue({ success: true });
      vi.mocked(createBusterSDK).mockReturnValue({
      ...
      // Should now initialize SDK since messageId is auto-generated
      expect(credentials.getCredentials).toHaveBeenCalled();
      expect(createBusterSDK).toHaveBeenCalledWith({
        apiKey: 'test-api-key',
        apiUrl: 'https://api.test.com',
    ...
    it('should not initialize SDK when credentials are not available', async () => {
