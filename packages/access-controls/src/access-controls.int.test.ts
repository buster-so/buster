...
describe("Access Controls Integration Tests - Organization Default Permission Group", () => {
	...
	afterEach(async () => {
		...
		await db
			...
			.where(
				inArray(
					permissionGroupsToIdentities.permissionGroupId,
					testIds.permissionGroupIds,
				),
			);
		...
		await db
			.delete(teamsToUsers)
			.where(inArray(teamsToUsers.userId, testIds.userIds));
		...
		await db
			.delete(dataSources)
			.where(inArray(dataSources.id, testIds.dataSourceIds));
		...
		await db
			...
			.where(inArray(organizations.id, testIds.organizationIds));
	});
	...
	describe("getPermissionedDatasets with org default", () => {
		...
		it("should return empty array when default permission group does not exist", async () => {
			...
			await db
				.delete(usersToOrganizations)
				.where(eq(usersToOrganizations.userId, newUserId));
		...
		it("should not return deleted datasets from default permission group", async () => {
			...
			const accessibleDatasets = await getPermissionedDatasets(
				testUserId,
				0,
				10,
			);
	...
	describe("Concurrent access path execution", () => {
		it("should handle all permission paths concurrently without conflicts", async () => {
			...
			await db
				.delete(permissionGroupsToIdentities)
				.where(
					eq(permissionGroupsToIdentities.permissionGroupId, otherGroupId),
				);
			await db
				.delete(permissionGroups)
				.where(eq(permissionGroups.id, otherGroupId));
		});
	});
});
...
describe("getPermissionedDatasets Integration Tests", () => {
	...
	describe("User Dataset Access - Full Access User", () => {
		it("should return datasets for user including sales_person dataset", async () => {
			...
			const salesPersonDataset = result.find(
				(dataset) => dataset.name === "sales_person",
			);
			...
			console.info(
				`Sales person dataset found: ${salesPersonDataset ? "YES" : "NO"}`,
			);
		});
		...
		it("should return datasets in alphabetical order", async () => {
			...
			const salesPersonIndex = result.findIndex(
				(d) => d.name === "sales_person",
			);
			...
			if (page1.length === 10) {
				...
				if (nextPage.length > 0) {
					...
					const intersection = new Set(
						Array.from(firstPageIds).filter((x) => nextPageIds.has(x)),
					);
	...
	describe("User Dataset Access - No Access User", () => {
		it("should return empty array for user with no dataset access", async () => {
			...
			console.info(
				`User ${noAccessUserId} has access to ${result.length} datasets (expected: 0)`,
			);
		});
	...
	describe("User Dataset Access - Limited Access User", () => {
		it("should verify limited access user has access to person_phone dataset only", async () => {
			...
			console.info(
				`User ${limitedAccessUserId} has access to ${result.length} datasets`,
			);
			...
			const personPhoneDataset = result.find(
				(dataset) => dataset.name === "person_phone",
			);
			const salesPersonDataset = result.find(
				(dataset) => dataset.name === "sales_person",
			);
			...
			console.info(
				`Person phone dataset found: ${personPhoneDataset ? "YES" : "NO"}`,
			);
			console.info(
				`Sales person dataset found: ${salesPersonDataset ? "YES" : "NO"}`,
			);
		...
		it("should handle pagination correctly for limited access user", async () => {
			...
			const allResults = await getPermissionedDatasets(
				limitedAccessUserId,
				0,
				100,
			);
	...
	describe("Error Handling", () => {
		it("should validate input parameters", async () => {
			// Test invalid UUID
			await expect(
				getPermissionedDatasets("invalid-uuid", 0, 50),
			).rejects.toThrow();

			// Test negative page
			await expect(
				getPermissionedDatasets(testUserId, -1, 50),
			).rejects.toThrow();

			// Test invalid page size
			await expect(getPermissionedDatasets(testUserId, 0, 0)).rejects.toThrow();
			await expect(
				getPermissionedDatasets(testUserId, 0, 1001),
			).rejects.toThrow();
		});
	});
});
