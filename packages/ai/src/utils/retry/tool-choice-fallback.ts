...
/**
 * Implements a tool choice fallback strategy
 * When 'required' tool choice fails repeatedly, try with 'auto' or 'none'
 */
export class ToolChoiceFallback {
  private static readonly FALLBACK_SEQUENCE: Array<'required' | 'auto' | 'none'> = [
    'required', 
    'auto', 
    'none'
  ];
  ...
  static getToolChoiceForRetry(
    ...
    const fallbackIndex = Math.min(retryAttempt, this.FALLBACK_SEQUENCE.length - 1);
    return this.FALLBACK_SEQUENCE[fallbackIndex];
  }

  /**
   * Creates a healing message explaining the tool choice fallback
   */
  static createFallbackMessage(
   ...
   */
  static shouldFallbackToolChoice(error: unknown): boolean {
