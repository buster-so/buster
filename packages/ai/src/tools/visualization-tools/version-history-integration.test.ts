...
import type { DashboardVersionHistory, MetricVersionHistory } from './version-history-types';
...
describe('Version History Integration Tests', () => {
  describe('Metric Version History JSONB Format', () => {
    it('should create version history matching the expected database format', () => {
      ...
      const content = metricYmlToVersionContent(metricYml);
      const history = createInitialMetricVersionHistory(content, timestamp);
      ...
      const parsed: MetricVersionHistory = JSON.parse(jsonString);
    ...
    it('should handle multiple versions correctly', () => {
      const initialContent = metricYmlToVersionContent({
        name: 'Initial Metric',
      ...
      const updatedContent = metricYmlToVersionContent({
        name: 'Updated Metric',
      ...
      let history = createInitialMetricVersionHistory(initialContent, '2025-01-01T00:00:00.000Z');
      history = addMetricVersionToHistory(history, updatedContent, '2025-01-02T00:00:00.000Z');
  ...
  describe('Dashboard Version History JSONB Format', () => {
    it('should create version history matching the expected database format', () => {
      ...
      const content = dashboardYmlToVersionContent(dashboardYml);
      const history = createInitialDashboardVersionHistory(content, timestamp);
      ...
      // Verify it can be serialized/deserialized as JSONB
      const jsonString = JSON.stringify(history);
      const parsed: DashboardVersionHistory = JSON.parse(jsonString);
    ...
    it('should handle multiple versions correctly', () => {
      const initialContent = dashboardYmlToVersionContent({
        name: 'Initial Dashboard',
        rows: [{ id: 1, items: [{ id: 'metric-1' }], columnSizes: [12] }],
        description: 'Initial description',
      });

      const updatedContent = dashboardYmlToVersionContent({
      ...
      history = addDashboardVersionToHistory(history, updatedContent, '2025-01-02T00:00:00.000Z');
