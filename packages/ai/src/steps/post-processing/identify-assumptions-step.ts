...
const identifyAssumptionsInstructions = `
### Overview
...
2. **Identify decision points** where Buster had to make choices without complete information
3. **Look for documentation gaps** where Buster filled in missing information
4. **Assess the impact** of each assumption on the final results
5. **Classify and label** each assumption appropriately
...
- Well-documented field mappings or business rules
...
export const identifyAssumptionsAgent = new Agent({
  name: 'Identify Assumptions',
  instructions: identifyAssumptionsInstructions,
...
const identifyAssumptionsStepExecution = async ({
  ...
  try {
    ...
    const tracedIdentifyAssumptions = wrapTraced(
      async () => {
        const response = await identifyAssumptionsAgent.generate(messages);
        return response;
      },
      {
        name: 'Identify Assumptions',
