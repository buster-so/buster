...
export const SQL_DIALECT_GUIDANCE = {
  ...
  - **Performance Tips**:
    - Use Delta Lake features: \`OPTIMIZE\` for file compaction, \`ZORDER BY\` for co-location
    - Cache frequently used tables: \`CACHE TABLE table_name\`
    - Use \`ANALYZE TABLE\` to update statistics for better query planning
    - Partition tables by commonly filtered columns (like date) for better performance
