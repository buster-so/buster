...
You MUST plan extensively before each function call, and reflect extensively on the outcomes of the previous function calls. DO NOT do this entire process by making function calls only, as this can impair your ability to solve the problem and think insightfully.`,
  requiredVariables: ['todaysDate', 'sqlDialectGuidance'],
  optionalVariables: [],
  schema: AnalysisPromptVariablesSchema,
};

export const injectAnalysisPrompt = createPromptInjector<AnalysisPromptVariables>(
  AnalysisPromptVariablesSchema
);

export const createAnalysisPrompt = (variables: unknown): string => {
  const validatedVariables = validateAnalysisPromptVariables(variables);
  return injectAnalysisPrompt(ANALYSIS_SYSTEM_PROMPT, validatedVariables);
};
