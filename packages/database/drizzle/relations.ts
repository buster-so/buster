...
import { organizations, apiKeys, users, teams, permissionGroups, terms, collections, dashboards, dashboardVersions, dataSources, termsSearch, datasetGroups, datasetPermissions, datasets, datasetGroupsPermissions, threadsDeprecated, messagesDeprecated, chats, messages, messagesToFiles, dashboardFiles, storedValuesSyncJobs, metricFiles, permissionGroupsToUsers, metricFilesToDatasets, termsToDatasets, datasetsToPermissionGroups, datasetsToDatasetGroups, threadsToDashboards, userFavorites, teamsToUsers, metricFilesToDashboardFiles, collectionsToAssets, permissionGroupsToIdentities, assetPermissions, usersToOrganizations } from "./schema";
...
export const organizationsRelations = relations(organizations, ({many}) => ({
	...
	termsSearches: many(termsSearch),
...
export const termsSearchRelations = relations(termsSearch, ({one}) => ({
	organization: one(organizations, {
		fields: [termsSearch.organizationId],
		references: [organizations.id]
	}),
}));
...
export const datasetsToPermissionGroupsRelations = relations(datasetsToPermissionGroups, ({one}) => ({
	dataset: one(datasets, {
...
export const metricFilesToDashboardFilesRelations = relations(metricFilesToDashboardFiles, ({one}) => ({
	metricFile: one(metricFiles, {
...
export const permissionGroupsToIdentitiesRelations = relations(permissionGroupsToIdentities, ({one}) => ({
	user_createdBy: one(users, {
