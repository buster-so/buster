import { pgTable, uuid, jsonb, text, timestamp, pgPolicy, varchar, index, foreignKey, unique, check, boolean, integer, doublePrecision, uniqueIndex, bigint, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const dieselSchemaMigrations = pgTable("__diesel_schema_migrations", {
	...
	pgPolicy("diesel_schema_migrations_policy", { as: "permissive", for: "all", to: ["authenticated"], using: sql`true` }),
]);
...
export const datasetGroups = pgTable("dataset_groups", {
	...
	pgPolicy("dataset_groups_policy", { as: "permissive", for: "all", to: ["authenticated"], using: sql`true` }),
]);
...
export const datasetPermissions = pgTable("dataset_permissions", {
	...
	index("dataset_permissions_permission_lookup_idx").using("btree", table.permissionId.asc().nullsLast().op("text_ops"), table.permissionType.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.organizationId],
			foreignColumns: [organizations.id],
	...
	pgPolicy("dataset_permissions_policy", { as: "permissive", for: "all", to: ["authenticated"], using: sql`true` }),
	check("dataset_permissions_permission_type_check", sql`(permission_type)::text = ANY (ARRAY[('user'::character varying)::text, ('dataset_group'::character varying)::text, ('permission_group'::character varying)::text])`),
]);
...
export const permissionGroupsToUsers = pgTable("permission_groups_to_users", {
	...
	primaryKey({ columns: [table.permissionGroupId, table.userId], name: "permission_groups_to_users_pkey"}),
	pgPolicy("permission_groups_to_users_policy", { as: "permissive", for: "all", to: ["authenticated"], using: sql`true` }),
]);
...
export const entityRelationship = pgTable("entity_relationship", {
	...
	primaryKey({ columns: [table.primaryDatasetId, table.foreignDatasetId], name: "entity_relationship_pkey"}),
]);
...
export const metricFilesToDatasets = pgTable("metric_files_to_datasets", {
	...
	primaryKey({ columns: [table.metricFileId, table.datasetId, table.metricVersionNumber], name: "metric_files_to_datasets_pkey"}),
]);
...
export const datasetsToPermissionGroups = pgTable("datasets_to_permission_groups", {
	...
	primaryKey({ columns: [table.datasetId, table.permissionGroupId], name: "datasets_to_permission_groups_pkey"}),
	pgPolicy("datasets_to_permission_groups_policy", { as: "permissive", for: "all", to: ["authenticated"], using: sql`true` }),
]);
...
export const datasetsToDatasetGroups = pgTable("datasets_to_dataset_groups", {
	...
	primaryKey({ columns: [table.datasetId, table.datasetGroupId], name: "datasets_to_dataset_groups_pkey"}),
	pgPolicy("datasets_to_dataset_groups_policy", { as: "permissive", for: "all", to: ["authenticated"], using: sql`true` }),
]);
...
export const metricFilesToDashboardFiles = pgTable("metric_files_to_dashboard_files", {
	...
	primaryKey({ columns: [table.metricFileId, table.dashboardFileId], name: "metric_files_to_dashboard_files_pkey"}),
]);
...
export const threadsToDashboards = pgTable("threads_to_dashboards", {
	...
	primaryKey({ columns: [table.threadId, table.dashboardId], name: "threads_to_dashboards_pkey"}),
]);
...
export const userFavorites = pgTable("user_favorites", {
	...
	primaryKey({ columns: [table.userId, table.assetId, table.assetType], name: "user_favorites_pkey"}),
]);
...
export const collectionsToAssets = pgTable("collections_to_assets", {
	...
	primaryKey({ columns: [table.collectionId, table.assetId, table.assetType], name: "collections_to_assets_pkey"}),
]);
...
export const permissionGroupsToIdentities = pgTable("permission_groups_to_identities", {
	...
	primaryKey({ columns: [table.permissionGroupId, table.identityId, table.identityType], name: "permission_groups_to_identities_pkey"}),
]);
...
export const assetPermissions = pgTable("asset_permissions", {
	...
	primaryKey({ columns: [table.identityId, table.identityType, table.assetId, table.assetType], name: "asset_permissions_pkey"}),
]);
...
export const usersToOrganizations = pgTable("users_to_organizations", {
	...
	primaryKey({ columns: [table.userId, table.organizationId], name: "users_to_organizations_pkey"}),
]);
