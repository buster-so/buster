...
-- 3. Safety Net: database → default_database (for all database types)
-- 4. Safety Net: schema → default_schema (for all database types that support schemas)
-- 5. Normalize data_sources.secret_id to use secret.id (UUID) instead of secret.name
--
...
BEGIN
  ...
  LOOP
    ...
    IF secret_name_to_id_map ? ds_record.secret_id::text THEN
      ...
      UPDATE data_sources
      SET
        secret_id = correct_secret_id::text,
        updated_at = now()
