...
async function getChatIdFromMessage(messageId: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Database query failed while finding message: ${dbError instanceof Error ? dbError.message : 'Unknown database error'}`);
  }
...
async function getAllMessagesForChat(chatId: string): Promise<Array<{
  id: string;
  ...
  try {
    ...
    throw new Error(`Database query failed while loading chat messages: ${dbError instanceof Error ? dbError.message : 'Unknown database error'}`);
  }
...
function combineRawLlmMessages(chatMessages: Array<{ rawLlmMessages: unknown }>): CoreMessage[] {
  ...
  try {
    ...
    throw new Error(`Failed to process conversation history: ${processingError instanceof Error ? processingError.message : 'Unknown processing error'}`);
  }
...
export async function getChatConversationHistory(input: ChatConversationHistoryInput): Promise<ChatConversationHistoryOutput> {
  try {
    ...
    throw error instanceof Error ? error : new Error(`Failed to get chat conversation history: ${String(error)}`);
  }
}