...
export const deployAutomationTasks = async (
  ...
  try {
    ...
    const permissionedRepositories = (
      githubIntegration.accessibleRepositories as GithubRepository[]
    ...
    for (const task of tasks) {
      ...
      if (!permissionedRepositories.includes(task.repository)) {
        throw new Error(
          `Repository ${task.repository} is not accessible to the Buster Agent Github app. Please add the repository before deploying automation tasks.`
        );
      }
    }
    ...
    try {
      ...
      throw new Error('Failed to execute deployment queries.');
    }
  } catch (error) {
